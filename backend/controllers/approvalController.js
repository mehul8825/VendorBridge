    ...
    if (entityType === 'quotation') {
      ...
      if (quote) {
        if (status === 'approved') {
          ...
          // Notify vendor that their bid was approved for PO release
          await Notification.create({
            userId: quote.vendor.userId,
            message: `Congratulations! Your bid for "${quote.rfq.title}" has been accepted. A Purchase Order is being generated by the Procurement department.`,
            type: 'success'
          });
          ...
          await ActivityLog.create({
            userId: req.user.id,
            action: 'BID_AWARDED',
            details: `Manager approved bid #${quote.id} for vendor '${quote.vendor.companyName}' (pending PO release)`
          });
        } else {
